...
class Apollo(BaseModel):
    ...
    def spec_band_split(self, input):
        ...
        for i in range(self.nband):
            ...
            denom = subband_power[-1].real  # 또는 denom = subband_power[-1].abs() 도 가능
            subband_spec_norm.append(torch.complex(this_spec.real / denom, this_spec.imag / denom))
    ...
    def feature_extractor(self, input):
        ...
        for i in range(self.nband):
            ...
            real_spec = concat_spec.real  # 또는 abs(concat_spec)
            subband_feature.append(self.BN[i](real_spec))
